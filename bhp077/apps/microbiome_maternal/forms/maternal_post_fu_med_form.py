...
class MaternalPostFuMedForm(BaseMaternalModelForm):
    def clean(self):
        ...
        if cleaned_data.get('has_taken_meds') == YES and not check_med_items:
            raise forms.ValidationError('You have indicated that medications were taken. Please provide them')
        return cleaned_data
...
class MaternalPostFuMedItemsForm(BaseMaternalModelForm):
    def clean(self):
        ...
        if cleaned_data.get('maternal_post_fu_med').has_taken_meds in ['Unknown', NO]:
            raise forms.ValidationError('You indicated that has taken meds was {} therefore you cannot provide '
                                        'medication. Please correct.'
                                        .format(cleaned_data.get('maternal_post_fu_med').has_taken_meds))
