...
from ..managers import PostnatalCallLogManager
from ..choices import CONTACT_TYPE, APPT_GRADING, APPT_LOCATIONS
from .postnatal_call_list import PostnatalCallList
from ..managers import PostnatalCallLogEntryManager


class PostnatalCallLog (BaseSyncUuidModel):

    postnatal_call_list = models.ForeignKey(PostnatalCallList)
    ...
    objects = PostnatalCallLogManager()

    def __unicode__(self):
        return '{} {}'.format(
            self.postnatal_call_list.first_name,
            self.postnatal_call_list.initials)

    def save(self, *args, **kwargs):
        super(PostnatalCallLog, self).save(*args, **kwargs)

    def natural_key(self):
        return self.postnatal_call_list.natural_key()

    class Meta:
        app_label = 'call'


class PostnatalCallLogEntry (BaseSyncUuidModel):

    postnatal_call_log = models.ForeignKey(PostnatalCallLog)
    ...
    objects = PostnatalCallLogEntryManager()

    def save(self, *args, **kwargs):
        super(PostnatalCallLogEntry, self).save(*args, **kwargs)

    def __unicode__(self):
        return '{} {} {}'.format(
            self.postnatal_call_log.postnatal_call_list.first_name,
            self.postnatal_call_log.postnatal_call_list.initials,
        )
